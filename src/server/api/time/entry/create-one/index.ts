...
export default defineEventHandler(async (event) => {
  ...
  if (method === "POST") {
    ...
    const dataJira = await createJiraEntry(entryBody);
    if(dataJira?.id){
      const data = await createEntry(entryBody, dataJira.id);
    }
    return dataJira;
  } else {
  ...
  async function createJiraEntry(entryBody: any){
    try{
      ...
      const worklogData = {
        ...
        started: new Date(entryBody.entryDate).toISOString().slice(0, -1) + "+0000",
